...
/*
 ...
 - https://socket.io/docs/v3/emitting-events/
    - Acknowledgements
        - callback (can use to swap player after a player moves based on the proper return of the 
event the last thing can be a call back telling the server who is next in order
...
//            for(let c in handRev){
//                if(handRev[c].number != this.firstPairRank && handRev[c].number != 
this.secondPairRank && this.twoPairKicker == 0 ){
...
//        //Bug: May not recognize a straight flush if there are 2 cards of one of the numbers in 
the straight, because handUnique only stores 1 of the suits.
//        let handUnique = []
//        for (let c in this.hand){
...
//                if((handUnique[i+1].number == handUnique[i].number + 1) && 
(handUnique[i+2].number == handUnique[i].number + 2) && (handUnique[i+3].number == 
handUnique[i].number + 3) && (handUnique[i+4].number == handUnique[i].number + 4)){
//                    this.rank = Rank.straight;
//                    this.straightHigh = handUnique[i+4].number;
//                    //check for straight flush
//                    if(handUnique[i].suit == handUnique[i+1].suit && handUnique[i].suit == 
handUnique[i+2].suit && handUnique[i].suit == handUnique[i+3].suit && handUnique[i].suit == 
handUnique[i+4].suit){
//                        straightFlush = true;
...
//}
//
///* maybe create a struct for each connection to be able to specify the ID of each socket to be 
able to reference each user*/
//
//gameCodeCounter = 1
//gameIdCounter = 0
//
...
//        //do this by
//
//        //need to pass in the round which will be created once create game was called so instance 
is already created
//
...
//
//
////Need to test whether this will work properly or not with the automatic updating or not if not 
then I will need a function to start the game
...
//                        io.to(gameList[i].userList[l].SocketID).emit("Player Turn", {msg: 
roundString});
//                    }
//
//                }
...
////this function will be called by determineAction function. It will cue the appropriate handler 
depending on what choices are available, which will send the input request to the player
////The input that is recieved will not come back to this function. When it is recieved, determine 
action will be called with the recieved input as an argument.
////Because input doesn't come back to here, determineAction or doDealerAction will need to handle 
the events associated with playerInput choices, like removing a player when they fold, or updating 
pot and stack values when a bet is made.
//
////everything done needs to be outputted to the UI via a handler, so we'll need a handler for 
everything like removing the player after they fold, dealing cards,changing pots, and any other 
changes that need to be observed in the UI. For now, all those actions are done directly, but we'll 
need to add calls to the handlers so that the UI gets updated too.
//
...
//    }
//    let input2 = 0;
//    console.log("Input for player with index %i: %s", playerIndex, 
r.allPlayers[playerIndex].name);
...
//    let pick;
//
//    //determineAction(pick, playerIndex, Number(input2)); this call will happen from input 
function
//}
//
//
...
//            r.highestBet += val; //make sure val = raise amount, not total bet for this to be 
right
...
//
//        } else if (r.dealerPlayed && r.playersIn.every(player => player.totalBet == 
r.highestBet)){
...
//
//        }
//        else if(prevAct == DealerAction.dealFlop || prevAct == DealerAction.dealTurn || prevAct 
== DealerAction.dealRiver){
